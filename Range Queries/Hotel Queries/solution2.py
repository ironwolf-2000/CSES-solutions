...
class SegmentTree:
    ...
    def set(self, k: int, x: int) -> None:
        ...
        self.tree[k] = [k - self.n + 1, x]
        k //= 2

        while k >= 1:
            self.tree[k] = max(self.tree[k * 2], self.tree[k * 2 + 1], key=lambda _: _[1])
            k //= 2

    def first_available_index(self, value: int) -> int:
        if self.tree[1][1] < value:
            return 0

        k = 1

        while k * 2 < len(self.tree):
            if value <= self.tree[k * 2][1]:
                k = k * 2
            else:
                k = k * 2 + 1

        self.set(k - self.n, self.tree[k][1] - value)
